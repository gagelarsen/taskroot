...
class TestTimeEntryViewPermissions:
    ...
    def test_staff_can_update_any_time_entry(self, staff_user, staff_profile, other_staff_user_profile, deliverable):
        """Test that staff can update any time entry (time entries no longer track staff)."""
        # Create time entry
    ...
    def test_staff_can_delete_any_time_entry(self, staff_user, staff_profile, other_staff_user_profile, deliverable):
        """Test that staff can delete any time entry (time entries no longer track staff)."""
        # Create time entry
        entry = DeliverableTimeEntry.objects.create(
            deliverable=deliverable,
            entry_date=date(2024, 1, 15),
